...
export function KbTextEntries({ challengeId }: KbTextEntriesProps) {
  ...
  const { data: kbData, isLoading } = useGetV1ChallengesIdKnowledgeBase(
    challengeId,
    { page: 1, perPage: 100 },
  );
  ...
  const updateMutation = usePutV1ChallengesIdKnowledgeBaseKbid({
    mutation: {
      onSuccess: () => {
        ...
        setEditingId(null);
        setEditingContent("");
        setFeedback({ type: "success", message: "Entrada atualizada" });
      },
      onError: (err) => {
        setFeedback({
          type: "error",
          message: err.message ?? "Erro ao atualizar entrada",
        });
      },
    },
  });
  ...
  const deleteMutation = useDeleteV1ChallengesIdKnowledgeBaseKbid({
    mutation: {
      onSuccess: () => {
        ...
        setFeedback({ type: "success", message: "Entrada removida" });
      },
      onError: (err) => {
        setFeedback({
          type: "error",
          message: err.message ?? "Erro ao remover entrada",
        });
      },
    },
  });
                        ...
                        <AlertDialogContent className="bg-slate-800 border-slate-700">
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription className="text-slate-400">
                              Tem certeza que deseja remover esta entrada da
                              knowledge base?
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
