...
function AssistantMessageContent({ content }: { content: string }) {
  ...
  const trimmed =
    normalizeAssistantMarkdown(content || "") || t("awaitingResponse");
...
function ChatPanel() {
  ...
  const messages = useMemo(() => {
    ...
    if (workSession?.interactions) {
      for (const i of workSession.interactions) {
        if (i.interactionType === "chat") {
          ...
          arr.push({
            role: "assistant",
            content: i.modelResponse || t("awaitingResponse"),
            key: `${i.id}-assistant`,
          });
        }
      }
    ...
    return arr;
  }, [workSession?.interactions, optimisticUserMessage, t]);
  ...
  const submitMessage = async () => {
    ...
    try {
      ...
      setChatError(err instanceof Error ? err.message : t("errors.sendMessage"));
    } finally {
